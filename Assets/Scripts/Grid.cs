...
public class Grid : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            PiecePrefab piecePrefab = piecePrefabs[i];
            if (!piecePrefabDict.ContainsKey(piecePrefab.type))
            {
                piecePrefabDict.Add(piecePrefab.type, piecePrefab.prefab);
            }
        }
    ...
    public bool FillStep()
    {
        ...
        for (int x = 0; x < xDim; x++)
        {
            ...
            if (pieceBelow.Type == PieceType.EMPTY)
            {
                ...
                piece.Init(x, -1, this, PieceType.NORMAL);
                piece.MovableComponent.Move(x, 0, fillTime);
                piece.ColorComponent.SetColor((ColorPiece.ColorType)Random.Range(0, piece.ColorComponent.NumColors));
                pieces[x, 0] = piece;
                movedPiece = true;
            }
        }
        ...
        if (piece.IsColoured())
        {
            ...
            for (int dir = 0; dir <= 1; dir++)
            {
                for (int xOffset = 1; xOffset < xDim; xOffset++)
                {
                    ...
                    if (pieces[x, newY].IsColoured() && pieces[x, newY].ColorComponent.Color == color)
                    {
                ...
                for (int i = 0; i < horizontalPieces.Count; i++)
                {
                    for (int dir = 0; dir <= 1; dir++)
                    {
                        for (int yOffset = 1; yOffset < yDim; yOffset++)
                        {
                            ...
                            if (pieces[horizontalPieces[i].X, y].IsColoured() && pieces[horizontalPieces[i].X, y].ColorComponent.Color == color)
                            {
            ...
            for (int dir = 0; dir <= 1; dir++)
            {
                for (int yOffset = 1; yOffset < yDim; yOffset++)
                {
                    ...
                    if (pieces[newX, y].IsColoured() && pieces[newX, y].ColorComponent.Color == color)
                    {
                ...
                for (int i = 0; i < verticalPieces.Count; i++)
                {
                    for (int dir = 0; dir <= 1; dir++)
                    {
                        for (int xOffset = 1; xOffset < xDim; xOffset++)
                        {
                            ...
                            if (pieces[x, verticalPieces[i].Y].IsColoured() && pieces[x, verticalPieces[i].Y].ColorComponent.Color == color)
                            {
                ...
                if (pieces[x, y].IsClearable())
                {
                    ...
                    if (match != null)
                    {
                        ...
                        if (specialPieceType != PieceType.COUNT)
                        {
                            ...
                            if ((specialPieceType == PieceType.ROW_CLEAR || specialPieceType == PieceType.COLUMN_CLEAR) && newPiece.IsColoured() && match[0].IsColoured())
                            {
    ...
    public void ClearColor(ColorPiece.ColorType color)
    {
        for (int x = 0; x < xDim; x++)
        {
            for (int y = 0; y < yDim; y++)
            {
                if (pieces[x, y].IsColoured() && (pieces[x, y].ColorComponent.Color == color || color == ColorPiece.ColorType.ANY))
                {
                    ClearPiece(x, y);
                }
            }
